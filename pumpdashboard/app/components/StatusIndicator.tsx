...
const PumpSVGComponent: React.FC<MySVGComponentProps> = ({ color, width, height }) => (
    <svg 
...
const StatusIndicator: React.FC<StatusIndicatorProps> = ({ label, value, size }) => {
    ...
    const color = statusColor[value] || 'grey';

    const isBlinking = value === 'BROKEN'; // Determine when to blink
    return (
        <div className={`${styles.statusIndicator} ${isBlinking ? 'blinking' : ''}`}>
            <PumpSVGComponent color={color}  width={size} height={size}/>
            <p className={styles.statusText} style={{ color: color }}>{label}: {value}</p> {/* Add this line */}
        </div>
    );
};
